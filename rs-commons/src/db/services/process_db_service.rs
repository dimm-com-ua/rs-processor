...
impl ProcessDbService {
    pub fn new() -> Self {
        ProcessDbService {
            repo: ProcessRepo::new(),
        }
    }

    pub async fn find_process_flow(
        &self,
        flow_code: String,
        tr: &Transaction<'_>,
    ) -> Result<ProcessFlow, DbServiceError> {
        match self.repo.find_flow(flow_code.as_str(), tr).await {
            Ok(process_flow) => Ok(process_flow),
    ...
    pub async fn find_starting_element(
        ...
        flow_id: uuid::Uuid,
        tr: &Transaction<'_>,
    ) -> Result<FlowElement, DbServiceError> {
    ...
    pub async fn get_out_routes(
        ...
        element_id: uuid::Uuid,
        tr: &Transaction<'_>,
    ) -> Result<Vec<FlowRoute>, DbServiceError> {
