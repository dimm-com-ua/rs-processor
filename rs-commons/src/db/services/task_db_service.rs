...
impl TasksDbService {
    ...
    pub async fn create_worker(&self, task_id: Uuid, element_id: Uuid, what: WorkerWhat, run_after: Option<DateTime<Utc>>, tr: &Transaction<'_>) -> Result<Uuid, ErrorDefinition> {
        let created_at = Utc::now();
        self.repo.create_worker(task_id,  element_id, what, created_at, run_after, tr).await
    }
    ...
    pub async fn set_current_flow_item(&self, task_id: Uuid, element_id: Uuid, tr: &Transaction<'_>) -> Result<(), ErrorDefinition> {
        self.repo.set_current_flow_item(task_id, element_id, tr).await
    }
