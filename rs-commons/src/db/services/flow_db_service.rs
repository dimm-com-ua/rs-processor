...
impl FlowDbService {
    ...
    pub async fn get_flow_item_arguments(
        ...
        tr: &Transaction<'_>,
    ) -> Result<Vec<FlowElementArgument>, DbServiceError> {
        match self.repo.get_flow_item_arguments(element_id, tr).await {
            Ok(args) => Ok(args
                .iter()
                .map(|(arg, dt)| FlowElementArgument::from_db(arg, dt))
                .collect()),
    ...
    pub async fn save_flow_item_out_variables(
        ...
        self.repo
            .save_flow_item_out_variables(task_id, element_id, arg_names, task_variables, tr)
            .await
    }
}
