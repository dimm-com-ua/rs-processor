...
impl FlowRepo {
    ...
    pub async fn save_flow_item_out_variables(&self,
                                              task_id: uuid::Uuid,
        ...
        for arg in arg_names {
            ...
            match tr.query(query, &[&task_id.clone(), &element_id.clone()]).await {
                Ok(_) => {}
        ...
        return if args_with_error.is_empty() {
            match self.get_flow_item_arguments(element_id.clone(), &tr).await {
                Ok(res) => { Ok(res) }
                Err(err) => { Err(ErrorDefinition::with_reason("Db error".to_string(), json!({"error": format!("{:?}", err)}))) }
            }
        } else {
            Err(
                ErrorDefinition::with_reason("Error saving results".to_string(), json!({"fields_with_errors": args_with_error}))
            )
        }
    }
}