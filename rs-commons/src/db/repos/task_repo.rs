...
impl TasksDbRepo {
    pub async fn create_task(
        ...
        match tr
            .query(query, &[&flow_uuid, &created_at, &current_flow_id])
            .await
        {
            Ok(rows) => {
                if let Some(row) = rows.last() {
                    ...
                    if args.is_some() {
                        for (arg, value) in args.unwrap().iter() {
                            let data_type = if let Some(elm_arg) =
                                element_args.iter().find(|x| x.name.clone() == arg.clone())
                            {
                            ...
                            if let Err(err) =
                                tr.query(query, &[&uuid, &arg, &data_type, value]).await
                            {
                                return Err(ErrorDefinition::with_reason(
                                    "Error inserting flow variable".to_string(),
                    ...
                    Err(ErrorDefinition::empty(
                        "Couldn't get uuid of the created task".to_string(),
                    ))
                }
            }
            Err(err) => Err(ErrorDefinition::with_reason(
                "Couldn't create task".to_string(),
                json!({ "error": format!("{:?}", err) }),
            )),
        }
    }
            ...
            .await
        {
            Ok(rows) => {
                if let Some(row) = rows.last() {
                    ...
                    Err(ErrorDefinition::empty(
                        "Couldn't get uuid of created worker".to_string(),
                    ))
                }
            }
            Err(err) => Err(ErrorDefinition::with_reason(
                "Couldn't create task worker".to_string(),
                json!({ "error": format!("{:?}", err) }),
            )),
        }
    }
    ...
    pub async fn get_task_variables(
        ...
        match tr.query(query, &[&task_id]).await {
            Ok(rows) => {
                let vars: Vec<TaskVariable> = rows
                    .iter()
                    .map(|x| {
            ...
            Err(err) => Err(ErrorDefinition::with_reason(
                "Couldn't get task variables".to_string(),
                json!({ "error": format!("{:?}", err) }),
            )),
        }
    }
    ...
    pub async fn set_current_flow_item(
        ...
        match tr.query(query, &[&element_id, &task_id]).await {
            Ok(_) => Ok(()),
            Err(err) => Err(ErrorDefinition::with_reason(
                "Couldn't update task current element".to_string(),
                json!({ "error": format!("{:?}", err) }),
            )),
        }
    }
    ...
    pub async fn clear_task(
        ...
        if let Err(err) = tr.query(query, &[&task_id]).await {
            return Err(ErrorDefinition::with_reason(
                "Couldn't delete task worker".to_string(),
                json!({ "error": format!("{:?}", err) }),
            ));
        }
        ...
        if let Err(err) = tr.query(query, &[&task_id]).await {
            return Err(ErrorDefinition::with_reason(
                "Couldn't delete task variables".to_string(),
                json!({ "error": format!("{:?}", err) }),
            ));
        }
        ...
        if let Err(err) = tr.query(query, &[&task_id]).await {
            return Err(ErrorDefinition::with_reason(
                "Couldn't delete task".to_string(),
                json!({ "error": format!("{:?}", err) }),
            ));
        }
        Ok(())
