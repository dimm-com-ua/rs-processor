...
impl FlowElementArgument {
    pub fn from_db(db_model: &FlowElementArgumentDb, dt: &DataTypeDb) -> Self {
        FlowElementArgument {
            ...
            direction: match db_model.direction.to_lowercase().trim() {
                "in" => ArgumentDirection::In,
                "out" => ArgumentDirection::Out,
                _ => ArgumentDirection::Undefined,
            },
...
impl FlowElement {
    pub async fn get_all_arguments(
        &self,
        dbs: &DbServices,
        tr: &Transaction<'_>,
    ) -> Result<Vec<FlowElementArgument>, ErrorDefinition> {
    ...
    pub async fn validate(
        ...
        match self.get_all_arguments(dbs, &tr).await {
            Ok(args) => {
                let agrs_not_found: Vec<&FlowElementArgument> = args
                    .iter()
                    .filter(|a| {
                ...
                if agrs_not_found.len() > 0 {
                    return Err(ErrorDefinition::with_reason(
                        "Couldn't start the process. One or more required field(s) was not passed"
                            .to_string(),
                        json!({"fields": agrs_not_found.iter().map(|x| x.name.clone()).collect::<Vec<String>>()}),
                    ));
                }
                let args_not_recognized: Vec<String> = args_to_process
                    .as_ref()
                    .unwrap()
                    .clone()
                    .into_keys()
                    .filter(|x| {
                        args.iter()
                            .filter(|a| a.name == x.clone() && a.direction == ArgumentDirection::In)
                            .count()
                            == 0
                    })
                    .collect();
                ...
                if args_not_recognized.len() > 0 {
                    return Err(ErrorDefinition::with_reason(
                        "Couldn't start the process. One or more fields couldn't be recognized"
                            .to_string(),
                        json!({ "fields": args_not_recognized }),
                    ));
                }

                let args_with_incorrect_handler: Vec<&FlowElementArgument> = args
                    .iter()
                    .filter(|a| app.dt(a.data_type.id.clone()).is_none())
                    .collect();
            ...
            Err(_) => Err(ErrorDefinition::empty("Not found".to_string())),
        }
    }
}
