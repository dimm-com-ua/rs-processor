...
impl TaskEngineService {
    ...
    pub async fn create_task(
        ...
        match db_client
            ...
            .await
        {
            Ok(tr) => match dbs.process.find_process_flow(task.flow, &tr).await {
                Ok(flow) => match dbs.process.find_starting_element(flow.id, &tr).await {
                    Ok(starting_element) => {
                        if let Err(err) = starting_element
                            .validate(task.arguments.clone(), dbs, &tr, &app)
                            .await
                        {
                            return Err(err);
                        }
                        match flow
                            .run_task(&starting_element, task.arguments, dbs, &tr, &app)
                            .await
                        {
                            Ok(task) => {
                                let now = Utc::now();
                                    ...
                                    .await
                                {
                                    return Err(ErrorDefinition::with_reason(
                                        "Error creating worker".to_string(),
                                ...
                                if let Err(err) = tr.commit().await {
                                    Err(ErrorDefinition::with_reason(
                                        "Error committing transaction".to_string(),
                                        json!({ "error": format!("{:?}", err) }),
                                    ))
                                } else {
                                    Ok(task)
            ...
            Err(err) => Err(ErrorDefinition::empty(format!("{:?}", err))),
        }
    }
}
