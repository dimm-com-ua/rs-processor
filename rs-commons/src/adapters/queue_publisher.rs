...
impl QueuePublisher {
            ...
            Ok(conn) => match conn.create_channel().await {
                Ok(channel) => {
                    match channel
                        ...
                        .await
                    {
                        Ok(_) => Ok(QueuePublisher {
                            channel: Arc::new(channel),
                            config: Arc::new(queue_config),
                        }),
                        Err(err) => Err(ErrorDefinition::with_error("Couldn't declare queue", err)),
                    }
                }
    ...
    pub async fn create_worker(&self, uuid: uuid::Uuid) -> Result<(), ErrorDefinition> {
        ...
        {
            Ok(_) => Ok(()),
            Err(err) => Err(ErrorDefinition::with_error("Couldn't publish message", err)),
        }
    }
}
